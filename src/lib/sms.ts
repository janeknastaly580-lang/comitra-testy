 ...
 * else: no Account SID, no API key, no Verify/Messaging Service SID, no message
 ...
 *
 * Codes are never generated, stored or seen by this file either — Twilio Verify
 * holds them, and the backend only ever reports "pending" or "approved".
 */
 ...
 * the judge-invite flow simply registers without an SMS step, exactly as it did
 * before Twilio was set up. Nothing breaks while the values are still blank.
 */
export async function smsVerificationAvailable(): Promise<boolean> {
  ...
  try {
    ...
    const data = (await res.json()) as { configured?: boolean; verify?: boolean };
    return data?.configured === true && data?.verify === true;
  } catch {
    return false;
  }
}
 ...
 * generated by Twilio Verify and never travels to this device.
 */
...
/**
 * Check the code the person typed. Resolves only when Twilio approves it, so a
 * success genuinely proves they hold the number; throws
...
export interface SmsSendInput {
  ...
  /** Values the template fills in: ownerName, goalNumber, link. Never free text. */
