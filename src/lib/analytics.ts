/**
 * Pactista Analytics: a pure, deterministic statistics engine.
 *
...
/** Render the full analysis as a printable plain-text Pactista report. */
export function buildReport(name: string, a: Analysis): string {
  ...
  push('');
  push(line);
  push('  Generated by Pactista · deterministic, goal-history analytics');
  push(line);
  return L.join('\n');
}
